...
router.get('/', async (req, res) => {
  try {
    const gastos = await knex('empresa_2024').select('*');
    res.json(gastos);
  } catch (err) {
...
router.post('/', async (req, res) => {
  ...
  try {
    const result = await knex('empresa_2024').insert({
      descricao,
      valor,
      data_vencimento,
      janeiro,
...
router.put('/:id/:mesPago', async (req, res) => {
  ...
  try {
    const result = await knex('empresa_2024')
      .where({ id })
...
router.delete('/:id', async (req, res) => {
  const { id } = req.params;
  try {
    const rowsDeleted = await knex('empresa_2024').where({ id }).del();
    if (rowsDeleted) {
      res.status(204).send();
    } else {
      res.status(404).json({ error: 'Gasto não encontrado' });
